...
instance (Intersectable a, Intersectable b) => Intersectable (Intersection a b) where
   ...
   firstIntersection r int@(Inter a b) = case (firstIntersection r a, firstIntersection r b) of
      (Nothing, Nothing) -> Nothing
      (Nothing, m'@(Just (p,_,_))) -> if a `contains` p then m' else Nothing
      (m@(Just (p,_,_)), Nothing) -> if b `contains` p then m else Nothing
      (i@(Just (p,_,_)), i'@(Just (p',_,_))) -> case (compare `on` distance (origin r)) p p' of
         LT -> if b `contains` p
               then i
               else firstIntersection (r {origin = p}) int
         _  -> if a `contains` p'
               then i'
               else firstIntersection (r {origin = p'}) int
